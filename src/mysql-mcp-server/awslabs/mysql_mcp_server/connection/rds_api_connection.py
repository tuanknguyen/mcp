...
class RDSDataAPIConnection(AbstractDBConnection):
    ...
    def _execute_readonly_query(
        ...
        try:
            # Begin transaction
            # Note: Readonly enforcement is handled by the server-side mutation
            # keyword detector (detect_mutating_keywords), which rejects DML
            # before it reaches the database. SET TRANSACTION READ ONLY cannot
            # be used here because begin_transaction already starts a transaction,
            # and MySQL does not allow changing transaction characteristics after
            # a transaction is in progress (Error 1568).
